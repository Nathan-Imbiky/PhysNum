...
private:
  ...
  */  
  ...
  {
      return m*L*L*thetadot*thetadot/2 + m*g*L*(1-cos(theta));
  }
  ...
  {
      double acc = -(g/L)*sin(theta) - (kappa/m)*(thetadot + (r*Omega/L)*cos(Omega*t_ -theta)) + (r*Omega*Omega/L)*sin(Omega*t_ -theta);

...
// DONE: Implement the relative permittivity epsilon_r(r).
//       Should allow for a trivial test case (trivial=true) 
double epsilon_r(/* DONE: add arguments */double r, double b, double R, bool trivial)
{
   // DONE: replace
    if(r<0) 
    {
     cout<<"Error epsilon_r : r<0"<<endl;
    }
    if(trivial) {return 1;}
    else if(r<b) {return 1;}
    else {return 3 + 6*((r-b)/(R-b));}
